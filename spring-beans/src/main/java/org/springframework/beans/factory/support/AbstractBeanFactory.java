...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		//直接尝试从缓存获取 或者 singletonFactories 中的 ObjectFactory 中获取
		...
		if (sharedInstance != null && args == null) {
			...
			//缓存没有正在创建的单例模式Bean
			//缓存中已经有已经创建的原型模式Bean
			//但是由于循环引用的问题导致实例化对象失败 原型模式的依赖检查
	...
	protected Object getObjectForBeanInstance(
		...
		// 书本： 对非 FactoryBean 不处理 现在我们有了bean实例，它可以是普通bean或FactoryBean。如果它是一个FactoryBean，我们就用它来创建一个bean实例，但如果用户想获取工厂实例而不是工厂的getObject方法对应的实例那么传入的nama应该加入前缀
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		...
		Object object = null;
		if (mbd == null) {
			//从Bean工厂缓存中获取给定名称的Bean实例对象  书本 ： 尝试从缓存中加载bean
			object = getCachedObjectForFactoryBean(beanName);
		}
		//让Bean工厂生产给定名称的Bean对象实例
		if (object == null) {
			// Return bean instance from factory.
			// 书本：到这里明确知道 beanInstance 一定是FactoryBean类型  对bean的转化
			...
			// 书本 ：将从Factory中解析的bean的工作委托给 getObjectFromFactoryBean
